...
def get_valid_bbox(points, jprob=None, input_ratio=1.0, relsize=1.5):
    # points (25,2)
    if jprob is None:
    ...
    if rw * input_ratio > rh:
        rh = rw * input_ratio
    elif rh / input_ratio > rw:
        rw = rh / input_ratio
...
def bbox_to_objposwin(bbox):
    ...
    return np.array([cx, cy], dtype=np.float32), np.array([wx, wy], dtype=np.float32)
...
def get_gt_bbox(pose, visible, image_size, input_ratio=1.0, scale=1.2, logkey=None):
    ...
    assert len(pose) == len(visible), \
        'pose and visible should have the same length'
    ...
    for i in idx:
        ...
        if len(temp) == 0:
            print('no visible joints')
            temp = pose[i, pose[i] > 0]

        if len(temp) > 0:
            b = get_valid_bbox(temp, input_ratio=input_ratio, relsize=scale)
            ...
            if logkey not in logkey_warn:
                print('No ground-truth bounding box, ' \
