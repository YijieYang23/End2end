...
import logging
import mmcv
# from reference.mmpose.mmpose.models.registry import BACKBONES
# from reference.mmpose.mmpose.models.backbones.resnet import BasicBlock, Bottleneck
# from reference.mmpose.mmpose.models.backbones.utils import load_checkpoint, channel_shuffle
from core.net.litehrnet.utils.utils import load_checkpoint
from core.net.litehrnet.utils.channel_shuffle import channel_shuffle
...
class Conv3x3Block(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, padding=1):
        ...
        self.conv = conv3x3(in_channels, out_channels, stride=stride, padding=padding)
...
class MultiFuseLayer(nn.Module):
    def __init__(self,channels=[80,160,320]):
        super(MultiFuseLayer, self).__init__()
        self.conv11 = Conv3x3Block(channels[0], channels[1], stride=2,padding=1)
        self.conv12 = Conv3x3Block(channels[1], channels[2], stride=2,padding=1)

        self.conv2  = Conv3x3Block(channels[1], channels[2], stride=2,padding=1)

        self.relu = nn.ReLU(inplace=True)

    def forward(self,inputs):
        assert len(inputs)==3 ,"Input length for MultiFuse error"
        inputs[0] = self.conv12(self.relu(self.conv11(inputs[0])))
        inputs[1] = self.conv2(inputs[1])
        out = torch.cat((inputs[0],inputs[1],inputs[2]),dim=1)
...
class LiteHRNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.coord_representation = self.cfg['MODEL']['COORD_REPRESENTATION']
        # head
        self.final_layer = nn.Conv2d(
            in_channels=self.extra['stages_spec']['num_channels'][0][0],
            out_channels=self.cfg['DATASET']['NUM_JOINTS'],
            kernel_size=self.extra['FINAL_CONV_KERNEL'],
            stride=1,
            padding=1 if self.extra['FINAL_CONV_KERNEL'] == 3 else 0
        )

        # self.pose_logsoftmax = cfg['MODEL']['LOGSOFTMAX'] if 'LOGSOFTMAX' in cfg['MODEL'] else False

        if self.coord_representation == 'simdr' or self.coord_representation == 'sa-simdr':
            head_input = int(cfg['MODEL']['IMAGE_SIZE'][0] * cfg['MODEL']['IMAGE_SIZE'][1] / 16)
            self.mlp_head_x = nn.Linear(head_input,
                                        int(cfg['MODEL']['IMAGE_SIZE'][0] * cfg['MODEL']['SIMDR_SPLIT_RATIO']))
            self.mlp_head_y = nn.Linear(head_input,
                                        int(cfg['MODEL']['IMAGE_SIZE'][1] * cfg['MODEL']['SIMDR_SPLIT_RATIO']))

        if self.cfg['MODEL']['MULTI_FUSE']:
            channels_list = self.stages_spec['num_channels'][-1]
            self.multi_fuse_layer = MultiFuseLayer([channels_list[-3],channels_list[-2],channels_list[-1]])
        num_channels_last = [
    ...
    def forward(self, x):
        ...
        x_ = rearrange(x_, 'b c h w -> b c (h w)')
        if self.coord_representation == 'heatmap':
            if self.cfg['MODEL']['MULTI_FUSE']:
                fuse_output = self.multi_fuse_layer(y_list[1:])
                return x_, fuse_output
            else:
                return x_
        elif self.coord_representation == 'simdr' or self.coord_representation == 'sa-simdr':
            pred_x = self.mlp_head_x(x_)
            pred_y = self.mlp_head_y(x_)
            # if self.pose_logsoftmax:
            #     pred_x = F.log_softmax(pred_x,dim=2)
            #     pred_y = F.log_softmax(pred_y,dim=2)
            if self.cfg['MODEL']['MULTI_FUSE']:
                fuse_output = self.multi_fuse_layer(y_list[1:])
                return torch.cat([pred_x,pred_y],dim=2), fuse_output
            else:
                return torch.cat([pred_x,pred_y],dim=2)
        else:
            raise
...
def test_demo():
    ...
    self = LiteHRNet(cfg, in_channels=3)
    self.eval()
    inputs = torch.rand(32, 3, 128, 128) # 通道数为 1/4,1/8,1/16,1/32
    output ,fuse= self.forward(inputs)
    print(output.shape)
    print(fuse.shape)
    print('test ok')
